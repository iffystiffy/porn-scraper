...
(() => {
  describe("Deeper", function () {
    ...
    it("Get frontpage", function (done) {
      ...
      lansky.frontPage(lansky.Site.DEEPER)
        .then(result => {
          expect(result)
          .to.be.an("object")
    ...
    it("Get stars page 1", function (done) {
      ...
      lansky.stars(lansky.Site.DEEPER)
      .then(result => {
    ...
    it("Get top rated videos page 10000", function (done) {
      ...
      lansky.topRated(lansky.Site.DEEPER, 10000)
      .then(result => {
    ...
    it("Get top rated videos page 1", function (done) {
      ...
      lansky.topRated(lansky.Site.DEEPER)
      .then(result => {
    ...
    it("Get latest videos page 1", function (done) {
      ...
      lansky.latest(lansky.Site.DEEPER)
      .then(result => {
        expect(result.videos.length).to.equal(12);
        done();
      })
    ...
    it("Get 'Jill Kassidy': Should contain Jill Kassidy and her video(s)", function (done) {
      ...
      lansky.star({
        ...
        .then(result => {
          ...
          expect(result.videos.map(v => v.title)).to.include("Forgive Me");
          expect(result)
    ...
    it("Search 'adria rae': Should contain 1 star", function (done) {
      this.timeout(15000);
    ...
    it("Get scene 'going-deeper'", function (done) {
      this.timeout(15000);

      lansky.scene("going-deeper", lansky.Site.DEEPER)
      .then(result => {
        expect(result.video.title).to.equal("Going Deeper");
    ...
    it("Get null scene", function (done) {
      ...
      lansky.scene("xyz", lansky.Site.DEEPER)
      .then(result => {
